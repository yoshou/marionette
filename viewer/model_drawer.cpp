...
static void draw_node(tinygltf::Model &model, const tinygltf::Node &node, resources_t &resources) {
  if (node.mesh > -1) {
    ...
    if (node.skin >= 0) {
      ...
      const tinygltf::Accessor &inv_bind_accessor = model.accessors[skin.inverseBindMatrices];

      const tinygltf::BufferView &inv_bind_buffer_view =
          model.bufferViews[inv_bind_accessor.bufferView];
      const tinygltf::Buffer &inv_bind_buffer = model.buffers[inv_bind_buffer_view.buffer];

      const auto data_ptr = reinterpret_cast<const float *>(
          &inv_bind_buffer.data[inv_bind_buffer_view.byteOffset + inv_bind_accessor.byteOffset]);
      ...
      for (std::size_t j = 0; j < skin.joints.size(); j++) {
        ...
        bind = glm::translate(glm::vec3(-bind_trans.x, bind_trans.y, -bind_trans.z)) *
               glm::toMat4(glm::quat(bind_rot.w, bind_rot.x, -bind_rot.y, bind_rot.z));
        const auto inv_bind = glm::inverse(bind);
        // std::cout << model.nodes[joint].name << ":" << glm::to_string(inv_bind) << std::endl;;
        // resources.transforms[j] = glm::inverse(mesh_node.transform) *
        // joint_node.compute_global_transform() * inv_bind;
...
static void update_node(tinygltf::Model &model, const tinygltf::Node &node,
                        resources_t &resources) {
  ...
  if (node.matrix.size() == 16) {
    ...
    if (node.scale.size() == 3) {
      transform =
          glm::scale(glm::vec3((float)node.scale[0], (float)node.scale[1], (float)node.scale[2])) *
          transform;
    }
    ...
    if (node.translation.size() == 3) {
      transform = glm::translate(glm::vec3((float)-node.translation[0], (float)node.translation[1],
...
model_drawer::model_drawer(std::string path) : data(new model_data()) {
  std::string err;
  std::string warn;
  ...
  for (std::size_t i = 0; i < data->model.textures.size(); i++) {
    const auto &tex = data->model.textures[i];
    {
      ...
      glTexImage2D(target, 0, format, image.width, image.height, 0, format, GL_UNSIGNED_BYTE,
                   &image.image.at(0));
...
float model_drawer::get_blend_weight(std::string name) const {
  return data->resources.blend_weights.find(name) != data->resources.blend_weights.end()
             ? data->resources.blend_weights.at(name)
             : 0.0f;
}
