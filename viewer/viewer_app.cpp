...
struct ir_viewer : public window_base {
  ...
  virtual void update() override {
    ...
    if (!model) {
      // model =
      // std::make_shared<model_drawer>("../data/Alicia_VRM/Alicia_VRM/Alicia/VRM/AliciaSolid.vrm");
      model = std::make_shared<model_drawer>(
          "../data/RAYNOS-chan-avatar_v1.0.2/RAYNOS-chan-avatar_v1.0.2/VRM/RAYNOS-chan_1.0.2.vrm");
    ...
    const std::map<std::string, std::string> parents = {
        ...
        {"upper_leg.L", "Spine"}, {"lower_leg.L", "upper_leg.L"}, {"foot.L", "lower_leg.L"},

        {"upper_leg.R", "Spine"}, {"lower_leg.R", "upper_leg.R"}, {"foot.R", "lower_leg.R"},
...

    {
      ...
      const auto retargeted_pose =
          retarget(local_norm_poses, model->get_bone_transforms(), parents);
...
int ir_viewer_main() {
...

  {
    const std::vector<std::tuple<std::string, float, float>> twist_bounds = {
        {"upper_leg.R", -90.f, 90.f}, {"lower_leg.R", -45.f, 45.f}, {"upper_leg.L", 0.f, 0.f},
        {"lower_leg.L", -45.f, 45.f}, {"foot.R", 0.f, 0.f},         {"foot.L", 0.f, 0.f},
        {"upper_arm.R", -90.f, 90.f}, {"lower_arm.R", -90.f, 90.f}, {"upper_arm.L", -90.f, 90.f},
        {"lower_arm.L", -90.f, 90.f}, {"hand.R", 0.f, 0.f},         {"hand.L", 0.f, 0.f},
        {"Chest", 0.f, 0.f},          {"Spine", 0.f, 0.f},          {"Neck", 0.f, 0.f},
    };
  ...
  model_detection.set_callback(
      [&model, &model_trackings, &model_trackings_mtx, frame_history](
          const model_instance_data &model_instance, uint32_t id, const frame_data_t &frame) {
        ...
        if (model_trackings.find(id) == model_trackings.end()) {
          ...
          if (frame_found) {
            const auto tracking =
                std::make_shared<model_tracking_worker>(model, model_instance, frame_found);
            tracking->start();
...
int main() { return ir_viewer_main(); }
