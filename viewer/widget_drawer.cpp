...
class nk_property_int {
 ...
 public:
  ...
  int get_min_value() const { return min; }
  int get_max_value() const { return max; }
  void set_min_value(int value) { min = value; }
  void set_max_value(int value) { max = value; }

  static std::shared_ptr<nk_property_int> create(std::string label, int value, int min, int max,
                                                 int step, float inc_per_pixel) {
    std::shared_ptr<nk_property_int> component(
        new nk_property_int(label, value, min, max, step, inc_per_pixel));
    return component;
  }
};
};  // namespace component
...
widget_drawer::widget_drawer() : pimpl(new widget_drawer_data()) {
  pimpl->frame_prop =
      component::nk_property_int::create("Frame", 2000, 0, std::numeric_limits<int>::max(), 1, 1);
