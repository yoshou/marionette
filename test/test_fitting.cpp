...
static std::tuple<std::vector<T>, std::vector<uint32_t>> blend_shapes(
    const std::vector<T> &betas, const std::vector<uint32_t> &betas_shape,
    const std::vector<float> &shape_disps, const std::vector<uint32_t> &shape_disps_shape) {
  const auto betas_steps = calculate_steps(betas_shape);
  const auto shape_disps_steps = calculate_steps(shape_disps_shape);

  const std::vector<uint32_t> blend_shapes_shape = {betas_shape[0], shape_disps_shape[0],
                                                    shape_disps_shape[1]};
  const auto blend_shapes_steps = calculate_steps(blend_shapes_shape);

  std::vector<T> blend_shapes(
      std::accumulate(blend_shapes_shape.begin(), blend_shapes_shape.end(), 1, std::multiplies{}));
    ...
    for (size_t m = 0; m < shape_disps_shape[0]; m++) {
      for (size_t k = 0; k < shape_disps_shape[1]; k++) {
        for (size_t l = 0; l < shape_disps_shape[2]; l++) {
          const auto dst_idx =
              b * blend_shapes_steps[0] + m * blend_shapes_steps[1] + k * blend_shapes_steps[2];
          blend_shapes[dst_idx] +=
              betas[b * betas_steps[0] + l * betas_steps[1]] *
              T(shape_disps[m * shape_disps_steps[0] + k * shape_disps_steps[1] +
                            l * shape_disps_steps[2]]);
        }
      }
    }
  }
...
static std::tuple<std::vector<T>, std::vector<uint32_t>> vertices2joints(
    const MatrixXd &j_regressor, const std::vector<T> &vertices,
  ...
  const std::vector<uint32_t> joints_shape = {
      vertices_shape[0], static_cast<uint32_t>(j_regressor.rows()), vertices_shape[2]};
    ...
    for (size_t j = 0; j < static_cast<uint32_t>(j_regressor.rows()); j++) {
      for (size_t k = 0; k < vertices_shape[2]; k++) {
        for (size_t i = 0; i < vertices_shape[1]; i++) {
          ...
          joints[dst_idx] +=
              vertices[b * vertices_steps[0] + i * vertices_steps[1] + k * vertices_steps[2]] *
              T(j_regressor(j, i));
        }
      }
    }
...
template <typename T>
inline void rodrigues(const T angle_axis[3], T result[9]) {
...
static std::tuple<std::vector<T>, std::vector<uint32_t>> batch_rodrigues(
    const std::vector<T> &rot_vecs, const std::vector<uint32_t> &rot_vecs_shape) {
...
static std::tuple<std::vector<T>, std::vector<T>> batch_rigid_transform(
    const std::vector<T1> &rot_mats, const std::vector<uint32_t> &rot_mats_shape,
    const std::vector<T2> &joints, const std::vector<uint32_t> &joints_shape,
  ...
  for (size_t i = 0; i < batch_size; i++) {
    for (size_t j = 0; j < joint_size; j++) {
      ...
      if (j != 0) {
        ...
        for (size_t m = 0; m < 4; m++) {
          for (size_t n = 0; n < 4; n++) {
            parent_transform_mat[m][n] =
                transform_mats[i * transform_mats_steps[0] + parents[j] * transform_mats_steps[1] +
                               m * 4 + n];
          }
        }
      }
      ...
      for (size_t m = 0; m < 4; m++) {
        for (size_t n = 0; n < 4; n++) {
          transform_mats[i * transform_mats_steps[0] + j * transform_mats_steps[1] + m * 4 + n] =
              global_transform_mat[m][n];
        }
      }

      for (size_t k = 0; k < 3; k++) {
        posed_joints[i * posed_joints_steps[0] + j * posed_joints_steps[1] +
                     k * posed_joints_steps[2]] = global_transform_mat[k][3];
      }
      ...
      for (size_t m = 0; m < 4; m++) {
        for (size_t n = 0; n < 4; n++) {
          rel_transform_mats[i * transform_mats_steps[0] + j * transform_mats_steps[1] + m * 4 +
                             n] = rel_transform_mat[m][n];
        }
      }
    }
  }
...
class smpl_model {
 ...
 public:
  smpl_model() {
    ...
    const auto v_template =
        load_dense_matrix<double>((param_dir / "SMPL_NEUTRAL_v_template.json").generic_string());
    const auto weights =
    ...
    const auto [shapedirs_data, shapedirs_shape] =
    ...
    const auto [kintree_table_data, kintree_table_shape] =
    ...
    j_shapedirs_shape = {static_cast<uint32_t>(j_regressor_ext.rows()), shapedirs_shape[1],
                         shapedirs_shape[2]};
    ...
    j_posedirs_shape = {static_cast<uint32_t>(j_regressor_ext.rows()), posedirs_shape[1],
                        posedirs_shape[2]};
  ...
  std::tuple<std::vector<T>, std::vector<T>> lbs(const std::vector<T1> &shapes_data,
    ...
    const auto [blend_shape, blend_shape_shape] =
        blend_shapes(shapes_data, shapes_shape, j_shapedirs, j_shapedirs_shape);

    std::vector<T> v_shaped(calculate_size(blend_shape_shape));
    std::transform(blend_shape.begin(), blend_shape.end(),
                   j_v_template.reshaped<Eigen::RowMajor>().begin(), v_shaped.begin(),
                   [](const auto a, const auto b) { return T(a) + T(b); });
    ...
    const auto [pose_offset, v_posed_shape] =
        blend_shapes(pose_feature, rot_mats_shape, j_posedirs, j_posedirs_shape);

    std::vector<T> v_posed(calculate_size(v_posed_shape));

    for (size_t b = 0; b < batch_size; b++) {
      std::transform(v_shaped.begin(), v_shaped.end(),
                     pose_offset.begin() + (b * pose_offset.size() / batch_size),
                     v_posed.begin() + (b * pose_offset.size() / batch_size),
                     [](const auto a, const auto b) { return a + T(b); });
    }

    const auto [j_transformed, transform_mats] =
        batch_rigid_transform<T>(rot_mats, rot_mats_shape, joints, joints_shape, parents);

    const auto joint_size = static_cast<uint32_t>(parents.size());
    std::vector<uint32_t> transform_mats_shape = {batch_size, joint_size, 16};
    const auto transform_mats_steps = calculate_steps(transform_mats_shape);

    std::vector<uint32_t> blend_transform_mats_shape = {
        batch_size, static_cast<uint32_t>(j_weights.rows()), 16};
      ...
      for (size_t i = 0; i < static_cast<uint32_t>(j_weights.rows()); i++) {
        for (size_t j = 0; j < 16; j++) {
          for (size_t k = 0; k < joint_size; k++) {
            const auto dst_idx = b * blend_transform_mats_steps[0] +
                                 i * blend_transform_mats_steps[1] +
                                 j * blend_transform_mats_steps[2];
            blend_transform_mats[dst_idx] +=
                j_weights(i, k) *
                transform_mats[b * transform_mats_steps[0] + k * transform_mats_steps[1] +
                               j * transform_mats_steps[2]];
          }
        }
      }
    }
    ...
    for (size_t b = 0; b < batch_size; b++) {
      for (size_t i = 0; i < v_posed_shape[1]; i++) {
        ...
        for (size_t j = 0; j < 3; j++) {
          T acc(0.0);
          for (size_t k = 0; k < 4; k++) {
            acc += T(blend_transform_mats[b * blend_transform_mats_steps[0] +
                                          i * blend_transform_mats_steps[1] + j * 4 + k]) *
                   v[k];
          }
...
struct limb_length_loss {
  ...
  limb_length_loss(const std::vector<float> &keypoints3d,
                   const std::vector<uint32_t> &keypoints3d_shape)
  ...
  inline T operator()(const T *const verts) const {
    ...
    for (size_t b = 0; b < keypoints3d_shape[0]; b++) {
      for (size_t i = 0; i < kintree.size(); i++) {
        const float v1[] = {keypoints3d[b * (keypoints3d_shape[1] * 4) + kintree[i].first * 4],
        ...
        const T e[] = {T(v2[0] - v1[0]), T(v2[1] - v1[1]), T(v2[2] - v1[2]), T(v2[3] - v1[3])};

        target[b * kintree.size() + i] =
            sqrt(e[0] * e[0] + e[1] * e[1] + e[2] * e[2] + e[3] * e[3]);
      }
    }
...
struct regression_loss {
 ...
 public:
  regression_loss(const std::vector<uint32_t> &values_shape) : values_shape(values_shape) {}
  ...
  inline T operator()(const T *const verts) const {
    ...
    for (size_t b = 0; b < keypoints3d_shape[0]; b++) {
      for (size_t i = 0; i < indices.size(); i++) {
        const T v1[] = {verts[b * (keypoints3d_shape[1] * 3) + indices[i] * 3],
                        verts[b * (keypoints3d_shape[1] * 3) + indices[i] * 3 + 1],
                        verts[b * (keypoints3d_shape[1] * 3) + indices[i] * 3 + 2]};

        const T v2[] = {T(keypoints3d[b * (keypoints3d_shape[1] * 4) + indices[i] * 4]),
...
struct smooth_loss {
 ...
 public:
  smooth_loss(const std::vector<uint32_t> &values_shape, const std::vector<float> &window_height,
              bool order2 = true)
        ...
        for (size_t b = 0; b < values_shape[0] - (k + 2); b++) {
          for (size_t i = 0; i < values_shape[1]; i++) {
            for (size_t j = 0; j < values_shape[2]; j++) {
              const auto d1 =
                  values[(b + k + 1) * (values_shape[1] * values_shape[2]) + i * values_shape[2] +
                         j] -
                  values[b * (values_shape[1] * values_shape[2]) + i * values_shape[2] + j];
              const auto d2 =
                  values[(b + k + 2) * (values_shape[1] * values_shape[2]) + i * values_shape[2] +
                         j] -
                  values[(b + 1) * (values_shape[1] * values_shape[2]) + i * values_shape[2] + j];
              const auto d = d2 - d1;
              sq_sum += d * d;
            }
          }
...
struct prior_loss {
 ...
 public:
  prior_loss(const std::vector<uint32_t> &values_shape, size_t num_gaussians = 8,
             double epsilon = 1e-16, bool use_merged = true)
    ...
    for (size_t m = 0; m < covs_shape[0]; m++) {
      const MatrixXd cov = Eigen::Map<MatrixXd>(covs.data() + (covs_shape[1] * covs_shape[2] * m),
                                                covs_shape[1], covs_shape[2]);
      const auto sqrdet = std::sqrt(cov.determinant());

      Eigen::Map<MatrixXd>(precisions.data() + (covs_shape[2] * covs_shape[1] * m), covs_shape[2],
                           covs_shape[1]) = cov.inverse();
    ...
    for (size_t b = 0; b < values_shape[0]; b++) {
      ...
      for (size_t m = 0; m < means_shape[0]; m++) {
        ...
        for (size_t j = 0; j < values_shape[1]; j++) {
          d[m * values_shape[1] + j] =
              values[b * values_shape[1] + j] - means[m * means_shape[1] + j];
        }
      }
      ...
      for (size_t m = 0; m < means_shape[0]; m++) {
        ...
        for (size_t i = 0; i < covs_shape[1]; i++) {
          ...
          for (size_t j = 0; j < covs_shape[2]; j++) {
            acc += precisions[m * (covs_shape[1] * covs_shape[2]) + i * covs_shape[2] + j] *
                   d[m * values_shape[1] + j];
          }
...
struct fit_shape_obj_func {
  ...
  fit_shape_obj_func(smpl_model *model, const std::vector<float> &keypoints3d,
                     const std::vector<uint32_t> &keypoints3d_shape,
                     const std::vector<uint32_t> &shapes_shape,
                     const std::vector<float> &poses_data, const std::vector<uint32_t> &poses_shape,
                     const std::vector<float> &rh_data, const std::vector<float> &th_data)
      : shapes_shape(shapes_shape),
        poses_data(poses_data),
        poses_shape(poses_shape),
        rh_data(rh_data),
        th_data(th_data),
        model(model),
        limb_length(keypoints3d, keypoints3d_shape),
        reg(shapes_shape) {}

  template <typename T>
  inline T operator()(const T *const shapes) const {
    std::vector<T> shapes_data(shapes, shapes + calculate_size(shapes_shape));
    const auto [verts, joints] = model->draw<T, T, float, float>(
...
struct init_rt_obj_func {
  ...
  init_rt_obj_func(smpl_model *model, const std::vector<float> &keypoints3d,
                   const std::vector<uint32_t> &keypoints3d_shape,
  ...
  inline T operator()(const T *const rh, const T *const th) const {
    ...
    const auto [verts, joints] = model->draw<T, float, float, T>(
        shapes_data, shapes_shape, poses_data, poses_shape, rh_data, th_data);
...
struct refine_pose_obj_func {
  ...
  refine_pose_obj_func(smpl_model *model, const std::vector<float> &keypoints3d,
                       const std::vector<uint32_t> &keypoints3d_shape,
                       const std::vector<float> &shapes_data,
                       const std::vector<uint32_t> &shapes_shape,
                       const std::vector<uint32_t> &poses_shape, const std::vector<float> &rh_data,
                       const std::vector<float> &th_data)
      : shapes_data(shapes_data),
        shapes_shape(shapes_shape),
        poses_shape(poses_shape),
  ...
  inline T operator()(const T *const poses, const T *const rh, const T *const th) const {
    ...
    const auto [verts, joints] = model->draw<T, float, T, T>(shapes_data, shapes_shape, poses_data,
                                                             poses_shape, rh_data, th_data);
    ...
    const auto smooth_loss =
        smooth_poses_loss * 100.0 + smooth_keypoints_loss * 10.0 + smooth_th_loss * 10.0;
    const auto smooth_loss_weight = 1.0;
    const auto prior_loss_weight = 0.1;
    const auto prior_loss = prior(poses_data.data());
    const auto loss = keypoints3d_loss * keypoints3d_loss_weight +
                      smooth_loss * smooth_loss_weight + prior_loss * prior_loss_weight;
    return loss;
  }
};
...
static double cubic_interpolate(const double x1, const double f1, const double g1, const double x2,
                                const double f2, const double g2, double xmin_bound,
                                double xmax_bound) {
  // Compute d1
  ...
  if (d2_square >= 0) {
    ...
    const auto min_pos = (x1 <= x2) ? (x2 - (x2 - x1) * ((g2 + d2 - d1) / (g2 - g1 + 2 * d2)))
...
static double zoom(const Func *obj_func, double f_low, double gtd_low, double alpha_low,
                   double f_high, double gtd_high, double alpha_high, const Eigen::VectorXd &params,
                   const double f, const Eigen::VectorXd &g, const Eigen::VectorXd &d,
                   std::size_t max_iteration = 25) {
  double c1 = 1e-3;
  double c2 = 0.9;
  ...
  for (std::size_t iter = 0; iter < max_iteration; iter++) {
    ...
    alpha = cubic_interpolate(alpha_low, f_low, gtd_low, alpha_high, f_high, gtd_high);
...
template <typename Func>
static double strong_wolfe(const Func *obj_func, const Eigen::VectorXd &params,
                           const Eigen::VectorXd &g, const Eigen::VectorXd &d, double alpha = 1.0,
                           std::size_t max_iteration = 25) {
  double c1 = 1e-3;
  double c2 = 0.9;
  ...
  for (std::size_t iter = 0; iter < max_iteration; iter++) {
    ...
    if (new_f > f + c1 * alpha * gtd || (iter > 0 && new_f >= f)) {
      // Zoom and return
      return zoom(obj_func, prev_f, prev_gtd, prev_alpha, new_f, new_gtd, alpha, params, f, g, d,
                  max_iteration);
    }
    ...
    const auto new_alpha =
        cubic_interpolate(prev_alpha, prev_f, prev_gtd, alpha, new_f, new_gtd, min_step, max_step);
...
class lbgfs_optimizer {
 ...
 public:
  lbgfs_optimizer() : s(history_size), y(history_size) {}
...
int main() {
...

  {
    ...
    std::shared_ptr<parameter_block> shapes_params =
        std::make_shared<parameter_block>(shapes_double.size(), shapes_double);
...

  {
    ...
    std::shared_ptr<parameter_block> rh_params =
        std::make_shared<parameter_block>(rh_data.size(), rh_double);
    std::shared_ptr<parameter_block> th_params =
        std::make_shared<parameter_block>(th_data.size(), th_double);
    th_params->offset = 0;
...

  {
    ...
    std::vector<double> rh_double(rh_data.begin(), rh_data.end());
    std::vector<double> th_double(th_data.begin(), th_data.end());
    std::shared_ptr<parameter_block> poses_params =
        std::make_shared<parameter_block>(poses_data.size(), poses_double);
    std::shared_ptr<parameter_block> rh_params =
        std::make_shared<parameter_block>(rh_data.size(), rh_double);
    std::shared_ptr<parameter_block> th_params =
        std::make_shared<parameter_block>(th_data.size(), th_double);
