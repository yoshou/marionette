...
[[maybe_unused]] static find_fit_result update_result(const find_fit_result &fit_result,
  ...
  const auto updated_pose =
      transform_by_param(glm::translate(fit_result.translation) * glm::mat4(fit_result.rotation),
                         rotation, translation);

  const auto updated_twist_angle =
      fit_result.twist_angle + glm::degrees(static_cast<float>(twist_angle));

  return find_fit_result(fit_result.anchor_index, glm::mat3(updated_pose),
...
static float compute_articulation_distance(const model_instance_data &model_instance,
  ...
  for (const auto &articulation_pair : articulation_pairs) {
    ...
    const auto [vec1, rot1, vec2, rot2] =
...
void motion_tracker::track_key_frame(
    const model_data &model, const frame_data_t &frame, const frame_data_t &keyframe,
    const model_instance_data &keyframe_clusters,
    const std::unordered_map<std::size_t, glm::vec3> &estimated_pos, bool verbose,
    std::size_t max_iter) {
  ...
  for (std::size_t iter = 0; iter < max_iter; iter++) {
    const auto cost_change = minimizer.update(iter, model, frame, marker_cloud, keyframe_clusters,
                                              estimated_pos, params, true, verbose);
...
void motion_tracker::track_interplated_frame(const model_data &model,
                                             const std::vector<camera_t> &cameras,
                                             const frame_data_t &frame,
...
model_instance_data motion_tracker::align_clusters(const model_instance_data &prev_model_instance,
  ...
  for (std::size_t i = 0; i < prev_clusters.size(); i++) {
    ...
    const auto cluster_pose =
        transform_to_target_pose(prev_cluster.cluster, prev_cluster.fit_result);
    ...
    for (std::size_t j = 0; j < prev_cluster.target.size(); j++) {
      ...
      const auto position = transform_twist_axis_angle(point.position, twist_angle, twist_axis,
                                                       twist_weight, rotation, translation);
    ...
    // cluster.fit_result = update_result(prev_cluster.fit_result, rotation, translation,
    // twist_angle);
...
void motion_tracker::track_frame(const model_data &model, const frame_data_t &frame,
...

  {
    ...
    for (const auto &cluster : aligned_clusters.clusters) {
      for (const auto &point : cluster.target) {
        if (updated.find(point.id) == updated.end()) {
          ...
          // std::cout << "diff " << point.id << " : " << glm::distance(predicted_position,
          // point.position) << std::endl;
    ...
    // debug_points.save("aligned_" +
    // std::to_string(marionette::utils::point_cloud_logger::get_logger().frame) + ".pcd");
  ...
  // std::cout << "Intraframe Dist : " << compute_articulation_distance(keyframe_clusters, params,
  // world) << std::endl;
}
