...
static void polyfit(const std::vector<double> &t, const std::vector<double> &v,
                    std::vector<double> &coeff, int order

) {
  // Create Matrix Placeholder of size n x k, n= number of datapoints, k = order of polynomial, for
  // exame k = 3 for cubic polynomial
...
struct polynomial_regression {
  ...
  polynomial_regression(std::size_t n, std::size_t order = 2) : values(n), order(order) {}
  ...
  glm::vec3 predict() const {
    ...
    std::transform(values.begin(), values.end(), std::back_inserter(x_values),
                   [](glm::vec3 value) { return value.x; });
    std::transform(values.begin(), values.end(), std::back_inserter(y_values),
                   [](glm::vec3 value) { return value.y; });
