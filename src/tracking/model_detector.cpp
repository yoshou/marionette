...
static bool solve_articulation(const model_data &model,
                               const std::map<std::string, std::vector<find_fit_result>> &results,
                               model_instance_data &registered_clusters,
                               float articulation_solved_threshold = 0.05f) {
  const auto add_articulation = [&](std::vector<articulation_results> &articulations,
                                    std::string parent_name, std::string child_name) {
    articulations.push_back(articulation_results(model.clusters.at(parent_name),
                                                 model.clusters.at(child_name),
                                                 results.at(parent_name), results.at(child_name)));
  };

  const auto add_result = [&](const std::string &cluster_name,
                              const std::map<std::string, std::size_t> &indices) {
    const auto &result = results.at(cluster_name);
    registered_clusters.clusters.push_back(
        rigid_cluster_instance(model.clusters.at(cluster_name), result[indices.at(cluster_name)]));
  };
...
static const rigid_cluster_instance *get_cluster_by_name(
    std::string name, const model_instance_data &registered_clusters) {
...
model_instance_data detect_model(const model_data &model, const frame_data_t &frame) {
  const std::vector<std::string> cluster_names = {
      "upper_leg.R", "lower_leg.R", "upper_leg.L", "lower_leg.L", "foot.R",
      "foot.L",      "upper_arm.R", "lower_arm.R", "upper_arm.L", "lower_arm.L",
      "hand.R",      "hand.L",      "Chest",       "Spine",       "Neck",
  };
...

  {
    ...
    for (std::size_t i = 0; i < num_queries; i++) {
      ...
      global_registration registration;
      registration.find_fits(cluster, target_cloud,
                             (cluster.max_twist_angle + cluster.min_twist_angle) * 0.5f,
                             cluster.min_twist_angle, cluster.max_twist_angle, result);
    }
#endif
