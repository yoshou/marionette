...
void model_detection_worker::set_callback(
    std::function<void(const model_instance_data&, std::uint32_t, const frame_data_t&)> func) {
  callback = func;
...
void model_detection_worker::process() {
  ...
  while (running) {
    ...
    std::cout << "=============== Detecting Frame : " << current_frame.frame_number
              << " =================" << std::endl;
