...
static triangle_feature extract_triangle_feature(const glm::vec3 &keypoint,
...
static inline float max3(glm::vec3 v) { return std::max(std::max(v.x, v.y), v.z); }

static float compute_feature_distance(const triangle_feature &feature1,
                                      const triangle_feature &feature2) {
