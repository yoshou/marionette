...
struct local_angle_error {
  ...
  template <typename T>
  bool operator()(const T* const parent_rotation, const T* const rotation,
                  const T* const child_rotation, T* residuals) const {
  ...
  static ceres::CostFunction* create() {
    return (
        new ceres::AutoDiffCostFunction<local_angle_error, 4, 4, 4, 4>(new local_angle_error()));
  }
};
...
struct local_angle_error2 {
  ...
  local_angle_error2(const glm::dquat& parent_rotation) : parent_rotation(parent_rotation) {}

  template <typename T>
  bool operator()(const T* const rotation, const T* const child_rotation, T* residuals) const {
    const T parent_rotation[] = {T(this->parent_rotation.x), T(this->parent_rotation.y),
                                 T(this->parent_rotation.z), T(this->parent_rotation.w)};
...
struct local_angle_error3 {
  ...
  local_angle_error3(const glm::dquat& child_rotation) : child_rotation(child_rotation) {}

  template <typename T>
  bool operator()(const T* const parent_rotation, const T* const rotation, T* residuals) const {
    const T child_rotation[] = {T(this->child_rotation.x), T(this->child_rotation.y),
                                T(this->child_rotation.z), T(this->child_rotation.w)};
...
struct articulation_error {
  ...
  template <typename T>
  bool operator()(const T* const parent_translation, const T* const parent_rotation,
                  const T* const child_translation, T* residuals) const {
    T point[3] = {T(static_cast<double>(this->local_child_position.x)),
...
struct articulation_error2 {
  ...
  template <typename T>
  bool operator()(const T* const child_translation, T* residuals) const {
    T p[3] = {T(static_cast<double>(this->local_child_position.x)),
              T(static_cast<double>(this->local_child_position.y)),
              T(static_cast<double>(this->local_child_position.z))};
...
struct local_1dof_constraint_error {
  ...
  template <typename T>
  bool operator()(const T* const parent_rotation, const T* const child_rotation,
                  T* residuals) const {
    Eigen::Map<const Eigen::Quaternion<T>> q1(parent_rotation);
...
struct local_1dof_constraint_error2 {
  ...
  template <typename T>
  bool operator()(const T* const child_rotation, T* residuals) const {
    const T parent_rotation[] = {T(this->parent_rotation.x), T(this->parent_rotation.y),
                                 T(this->parent_rotation.z), T(this->parent_rotation.w)};
...
struct local_1dof_constraint_error3 {
  ...
  local_1dof_constraint_error3(const glm::dquat& child_rotation) : child_rotation(child_rotation) {}

  template <typename T>
  bool operator()(const T* const parent_rotation, T* residuals) const {
    const T child_rotation[] = {T(this->child_rotation.x), T(this->child_rotation.y),
                                T(this->child_rotation.z), T(this->child_rotation.w)};
...
void estimate_finger_pose(std::map<std::string, glm::mat4>& poses, const model_data& model) {
  ...
  std::vector<std::string> bones = {"Little Intermediate.R", "Ring Intermediate.R",
                                    "Middle Intermediate.R", "Index Intermediate.R",
                                    "Thumb Intermediate.R",  "Little Proximal.R",
                                    "Ring Proximal.R",       "Middle Proximal.R",
                                    "Index Proximal.R",      "Thumb Proximal.R"};

  std::vector<std::string> target_bones = {"Little Distal.R", "Ring Distal.R", "Middle Distal.R",
                                           "Index Distal.R", "Thumb Distal.R"};
...

  {
    for (const auto& [bone, parent_bone, child_bone] : interpolations) {
      if (std::find(bones.begin(), bones.end(), parent_bone) == bones.end()) {
        ...
        ceres::CostFunction* cost_function = local_angle_error2::create(parent_pose);

        ceres::LossFunction* loss = nullptr; /* squared loss */

        problem.AddResidualBlock(cost_function, loss, rotation_param, child_rotation_param);
        ...
        ceres::CostFunction* cost_function = local_angle_error3::create(child_pose);

        ceres::LossFunction* loss = nullptr; /* squared loss */

        problem.AddResidualBlock(cost_function, loss, parent_rotation_param, rotation_param);
      } else {
        double* parent_rotation_param = bone_rotations.at(parent_bone);
        double* rotation_param = bone_rotations.at(bone);
        double* child_rotation_param = bone_rotations.at(child_bone);

        ceres::CostFunction* cost_function = local_angle_error::create();
...

  {
    for (const auto& [child_bone, parent_bone] : joints) {
      ...
      if (std::find(bones.begin(), bones.end(), parent_bone) == bones.end()) {
        ...
        ceres::CostFunction* cost_function = local_1dof_constraint_error2::create(parent_pose);

        ceres::LossFunction* loss = nullptr; /* squared loss */

        problem.AddResidualBlock(cost_function, loss, child_rotation_param);
      } else if (std::find(bones.begin(), bones.end(), child_bone) == bones.end()) {
        double* parent_rotation_param = bone_rotations.at(parent_bone);
        const glm::dquat child_pose = glm::quat_cast(poses.at(child_bone));

        ceres::CostFunction* cost_function = local_1dof_constraint_error3::create(child_pose);
        ...
        ceres::CostFunction* cost_function = local_1dof_constraint_error::create();

        ceres::LossFunction* loss = nullptr; /* squared loss */

        problem.AddResidualBlock(cost_function, loss, parent_rotation_param, child_rotation_param);
      }
    }
  }
  ...
  for (const auto& [target_bone, parent_bone] : joints) {
    if (std::find(bones.begin(), bones.end(), parent_bone) != bones.end()) {
      const auto bone_pose =
          std::find_if(model.bones.begin(), model.bones.end(),
                       [&](const auto& bone) { return bone.name == target_bone; });
      const auto parent_bone_pose =
          std::find_if(model.bones.begin(), model.bones.end(),
                       [&](const auto& bone) { return bone.name == parent_bone; });
      const auto bone_position =
          poses.at(parent_bone) * glm::inverse(parent_bone_pose->pose) * bone_pose->pose[3];
