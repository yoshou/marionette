...
std::map<std::string, glm::mat4> retarget_model::compute_normalized_pose(
    const std::map<std::string, glm::mat4>& pose) const {
  ...
  for (auto iter = pose.begin(); iter != pose.end(); iter++) {
    ...
    const auto transform = glm::translate(glm::vec3(pose[3])) *
                           glm::toMat4(glm::quat_cast(pose) * glm::inverse(glm::quat_cast(tpose)));
...
glm::mat4 get_bone_global_transform(const std::string& name,
...
glm::mat4 get_bone_local_transform(const std::string& name,
                                   const std::map<std::string, std::string>& parents,
                                   const std::map<std::string, glm::mat4>& transforms) {
  const auto pose = transforms.at(name);
  const auto parent_pose =
      (parents.find(name) != parents.end() && transforms.find(parents.at(name)) != transforms.end())
          ? transforms.at(parents.at(name))
          : glm::mat4(1.0f);
  return glm::inverse(parent_pose) * pose;
}

std::map<std::string, glm::mat4> retarget(const std::map<std::string, glm::mat4>& pose,
                                          const std::map<std::string, glm::mat4>& dst_pose,
                                          const std::map<std::string, std::string>& parents) {
        ...
        if (parents.find(name) == parents.end())  // root bone
        {
          for (std::size_t i = 0; i < 4; i++) {
            for (std::size_t j = 0; j < 3; j++) {
              transform[static_cast<glm::mat4::length_type>(i)]
                       [static_cast<glm::vec4::length_type>(j)] =
                           trans[static_cast<glm::mat4::length_type>(i)]
                                [static_cast<glm::vec4::length_type>(j)];
            }
          }
        } else {
