...
static inline glm::vec3 twist(const weighted_point &point, const glm::mat4 &pose,
                              const glm::mat4 &inv_pose, float angle) {
  const auto weight = point.weight;
  const auto rotation =
      glm::slerp(glm::quat(glm::vec3(0.f)),
                 glm::angleAxis(glm::radians(angle), glm::vec3(0.f, 1.f, 0.f)), weight);
...
static inline glm::vec3 rotate_local(const glm::vec3 &point, const glm::mat4 &pose,
...
static glm::vec3 transform_quat(const glm::vec3 &p, const T *rotation, const T *translation) {
  ...
  const glm::vec3 trans(static_cast<float>(translation[0]), static_cast<float>(translation[1]),
                        static_cast<float>(translation[2]));
...
static line3 transform_axis_angle(const line3 &l, const T *axis_angle, const T *translation) {
  ...
  if (angle > std::numeric_limits<float>::epsilon()) {
    const auto axis = glm::normalize(axis_angle_vec);
    const auto quat = glm::angleAxis(angle, axis);
    return line3{quat * l.origin + trans_vec, glm::normalize(quat * l.direction)};
  } else {
    return line3{l.origin + trans_vec, glm::normalize(l.direction)};
  }
}
...
static line3 transform_quat(const line3 &l, const T *rotation, const T *translation) {
  ...
  return line3{quat * l.origin + trans, glm::normalize(quat * l.direction)};
}
...
static inline weighted_point transform_coordinate(const weighted_point &point,
                                                  const glm::mat4 &transform) {
  return weighted_point{glm::vec3(transform * glm::vec4(point.position, 1.f)), point.weight,
                        point.id};
}

template <typename T>
static inline std::vector<T> transform_coordinate(const std::vector<T> &positions,
                                                  const glm::mat4 &transform) {
  std::vector<T> result;
