...
struct rigid_cluster {
  ...
  rigid_cluster(const std::string &name, const glm::mat4 &pose, float min_twist_angle,
                float max_twist_angle)
      : name(name),
        pose(pose),
  ...
  void load(std::string path) {
    ...
    for (std::size_t i = 0; i < objects.size(); i++) {
      ...
      for (const auto &[bone_name, weight] : obj.weights) {
        ...
        if (obj.name == anchor[bone_name]) {
          cluster.points.insert(cluster.points.begin(), weighted_point{obj.position, weight, i});
          markers.push_back(obj.position);
        } else {
          cluster.points.push_back(weighted_point{obj.position, weight, i});
    ...
    for (std::size_t i = 0; i < objects.size(); i++) {
      ...
      if (bone_names.size() > 1) {
        ...
        const auto point1_iter = std::find_if(cluster1.points.begin(), cluster1.points.end(),
                                              [i](const weighted_point &p) { return p.id == i; });
        const auto point2_iter = std::find_if(cluster2.points.begin(), cluster2.points.end(),
                                              [i](const weighted_point &p) { return p.id == i; });

        constraints.push_back(position_constraint{
            cluster_point_identifier{bone_names[0], static_cast<std::size_t>(std::distance(
                                                        cluster1.points.begin(), point1_iter))},
            cluster_point_identifier{bone_names[1], static_cast<std::size_t>(std::distance(
                                                        cluster2.points.begin(), point2_iter))},
        });
      }
    }
  }
