...
class frame_cursor {
 ...
 public:
  frame_cursor(const frame_data_t& frame) : frame(frame) {}

  const frame_data_t& get_frame() const { return frame; }
...
class frame_queue {
 public:
  ...
  frame_queue(size_t max_frame) : max_frame(max_frame) {}
  ...
  std::shared_ptr<frame_cursor> find(uint64_t frame_number) const {
    std::lock_guard lock(mtx);
    const auto found =
        std::find_if(frames.begin(), frames.end(), [frame_number](const auto& frame) {
          return frame->get_frame().frame_number == frame_number;
        });
    if (found != frames.end()) {
      return *found;
    }
