...
namespace glm {
static void to_json(nlohmann::json &j, const glm::vec2 &v) { j = {v.x, v.y}; };
static void from_json(const nlohmann::json &j, glm::vec2 &v) {
  v.x = j[0].get<float>();
  v.y = j[1].get<float>();
}
static void to_json(nlohmann::json &j, const glm::vec3 &v) { j = {v.x, v.y, v.z}; };
static void from_json(const nlohmann::json &j, glm::vec3 &v) {
  v.x = j[0].get<float>();
  v.y = j[1].get<float>();
  v.z = j[2].get<float>();
}
static void to_json(nlohmann::json &j, const glm::vec4 &v) { j = {v.x, v.y, v.z, v.w}; };
