...
static void transform_to_target(const rigid_cluster &cluster, const find_fit_result &result,
  ...
  for (std::size_t i = 0; i < cluster.points.size(); i++) {
    const auto twisted_pos =
        twist(cluster.points[i], cluster.pose, glm::inverse(cluster.pose), twist_angle);
...
static std::vector<weighted_point> find_target_points(const rigid_cluster &cluster,
  ...
  find_nearest_pair(rel_source_points, rel_source_pose, glm::inverse(rel_source_pose),
                    target_points, translation, rotation, pairs, twist_angle);

  std::vector<weighted_point> paired_target_points;
  for (auto [i, j] : pairs) {
    paired_target_points.push_back(
        weighted_point{target_points[j], cluster.points[i].weight, cluster.points[i].id});
  }
...
static std::vector<weighted_point> find_source_points(const rigid_cluster &cluster,
  ...
  for (auto [i, j] : pairs) {
    ...
    const auto source_point =
        twist(weighted_point{glm::transpose(rotation) * (target_point - translation),
                             cluster.points[i].weight, cluster.points[i].id},
              rel_source_pose, glm::inverse(rel_source_pose), -twist_angle) +
        anchor_pos;
    paired_target_points.push_back(
        weighted_point{source_point, cluster.points[i].weight, cluster.points[i].id});
  }
  return paired_target_points;
}
