...
template <typename T>
static T find_nearest_neighbor_point(const T &point, const std::vector<T> &points,
...
template <typename T>
static std::size_t find_nearest_neighbor_point_index(
    const T &point, const std::vector<T> &points, std::vector<float> &dists,
    float threshold = std::numeric_limits<float>::max()) {
  if (points.size() == 0) {
    return points.size();
  }
