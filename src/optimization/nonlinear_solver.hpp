...
static void gradient(ResidualFunc **residuals, std::size_t num_residuals,
                     ConstraintFunc **constraints, std::size_t num_constraints, const T *params,
                     std::size_t num_params, T *nabla) {
...
template <typename ResidualFunc, typename ConstraintFunc, typename T>
static void hessian(ResidualFunc **residuals, std::size_t num_residuals,
                    ConstraintFunc **constraints, std::size_t num_constraints, const T *params,
                    std::size_t num_params, T *nabla, T *H) {
...
static void hessian(Func **funcs, std::size_t num_funcs, const T *params, std::size_t num_params,
                    T *nabla, T *H) {
  std::vector<hyper_dual_t<T>> v(num_params);
...
template <typename Func, typename T>
static void jacobi(Func **funcs, std::size_t num_funcs, const T *params, std::size_t num_params,
                   T *J) {
  std::vector<dual_t<T>> v(num_params);
...
template <typename ResidualFunc, typename ConstraintFunc, typename T>
static optimization_result solve_newton_method(ResidualFunc **residuals, std::size_t num_residuals,
                                               ConstraintFunc **constraints,
                                               std::size_t num_constraints, T *params,
  ...
  for (std::size_t iter = 0; iter < max_iteration; iter++) {
    hessian(residuals, num_residuals, constraints, num_constraints, params, num_params,
            nabla.data(), H.data());
...
template <typename ResidualFunc, typename ConstraintFunc, typename T>
static T eval(ResidualFunc **residuals, std::size_t num_residuals, ConstraintFunc **constraints,
              std::size_t num_constraints, const T *params) {
  auto value = T{0};
  for (std::size_t i = 0; i < num_residuals; i++) {
...
static double armijo(ResidualFunc **residuals, std::size_t num_residuals,
                     ConstraintFunc **constraints, std::size_t num_constraints,
                     const Eigen::VectorXd &params, const Eigen::VectorXd &grad,
                     const Eigen::VectorXd &d, std::size_t max_iteration = 10000) {
  auto alpha = 1.0;
  auto rho = 0.5;
  double c1 = 1e-3;
  ...
  for (std::size_t iter = 0; iter < max_iteration; iter++) {
    ...
    if (eval(residuals, num_residuals, constraints, num_constraints, step_params.data()) <=
        eval(residuals, num_residuals, constraints, num_constraints, params.data()) +
            c1 * grad.dot(d)) {
      break;
    } else {
      alpha = alpha * rho;
...
template <typename ResidualFunc, typename ConstraintFunc, typename T>
static optimization_result solve_semi_newton_method(
    ResidualFunc **residuals, std::size_t num_residuals, ConstraintFunc **constraints,
    std::size_t num_constraints, T *params, std::size_t num_params, double terminate_thresold,
    std::size_t max_iteration) {
  optimization_result result;
  ...
  for (std::size_t iter = 0; iter < max_iteration; iter++) {
    ...
    gradient(residuals, num_residuals, constraints, num_constraints, params_v.data(), num_params,
             grad_v.data());

    Eigen::VectorXd d = B.inverse() * -grad_v;

    const auto alpha =
        armijo(residuals, num_residuals, constraints, num_constraints, params_v, grad_v, d);

    Eigen::VectorXd next_params_v = params_v + alpha * d;

    // Update B
    Eigen::VectorXd next_grad_v(num_params);
    gradient(residuals, num_residuals, constraints, num_constraints, next_params_v.data(),
             num_params, next_grad_v.data());
    ...
    const auto error =
        eval(residuals, num_residuals, constraints, num_constraints, params_v.data());
...
class function_term {
 public:
  virtual double eval(const double *params, std::size_t num_params) const { return 0.0; }
  ...
  virtual bool hessian(const double *params, std::size_t num_params, double *nabla,
                       double *H) const {
    return false;
  }
...
struct parameter_block {
  ...
  parameter_block(size_t size)
      : offset(std::numeric_limits<std::size_t>::max()), size(size), default_values{} {}
  ...
  parameter_block() : offset(std::numeric_limits<std::size_t>::max()), size(0), default_values{} {}
};
...
class auto_diff_function_term : public function_term {
 ...
 public:
  ...
  virtual bool add_grad(const double *params, std::size_t num_params,
                        double *nabla) const override {
    const auto v = std::make_unique<dual_t<value_type>[]>(num_params);
    for (std::size_t k = 0; k < num_params; k++) {
      v[k] = dual_t<value_type>(params[k], value_type{0});
    }

    const auto grad_params = [this, nabla,
                              &v](const std::shared_ptr<parameter_block> &param_block) {
      if (param_block->size == 3) {
        ...
        for (std::size_t i = param_block->offset; i < param_block->offset + param_block->size;
             i++) {
          v[i].b = value_type{1};
  ...
  virtual bool hessian(const double *params, std::size_t num_params, double *nabla,
    ...
    const auto hessian1_params = [this, params, num_params, nabla, H,
                                  &v](const std::shared_ptr<parameter_block> &param_block) {
      for (std::size_t i = param_block->offset; i < param_block->offset + param_block->size; i++) {
        for (std::size_t k = 0; k < num_params; k++) {
        ...
        const auto hessian2_params = [this, params, num_params, H, &v,
                                      i](const std::shared_ptr<parameter_block> &param_block) {
          for (std::size_t j = param_block->offset; j < param_block->offset + param_block->size;
               j++) {
            if (j <= i) {
              continue;
            }
            ...
            for (std::size_t k = 0; k < num_params; k++) {
              v[k] =
                  hyper_dual_t<value_type>(params[k], value_type{0}, value_type{0}, value_type{0});
            }
            v[i].b = value_type{1};
            v[j].c = value_type{1};
  ...
  virtual bool add_grad(dual_t<value_type> *v, std::size_t num_params,
                        double *nabla) const override {
    const auto grad_params = [this, nabla,
                              &v](const std::shared_ptr<parameter_block> &param_block) {
      if (param_block->size == 3) {
...
class function {
 ...
 public:
  ...
  function(std::shared_ptr<Terms>... terms) : terms{terms...} {}

  function(const std::vector<std::shared_ptr<function_term>> &terms) : terms(terms) {}
...
static double armijo(const function &obj_func, const Eigen::VectorXd &params,
                     const Eigen::VectorXd &grad, const Eigen::VectorXd &d,
...
static optimization_result solve_lbfgs_method(const function &obj_func, double *params,
                                              std::size_t num_params, double terminate_thresold,
                                              std::size_t max_iteration, std::size_t max_m) {
  optimization_result result;
...
class problem {
 ...
 public:
  problem(const function &obj_func) : obj_func(obj_func) {}

  void set_option(std::string prop, double value) { options[prop] = value; }

  void add_ineq_constraint(const function &func) { ineq_constraints.push_back(func); }
  void add_eq_constraint(const function &func) { eq_constraints.push_back(func); }
  ...
  optimization_result solve() {
...

    {
      ...
      return solve_lbfgs_method(obj_func, params.data(), params.size(), terminate_thresold,
                                max_iteration, max_m);
    }
  }
...
template <typename ResidualFunc, typename ConstraintFunc, typename T>
static optimization_result solve_lbfgs_method(ResidualFunc **residuals, std::size_t num_residuals,
                                              ConstraintFunc **constraints,
                                              std::size_t num_constraints, T *params,
                                              std::size_t num_params, double terminate_thresold,
                                              std::size_t max_iteration, std::size_t max_m) {
...
template <typename Func, typename T>
static optimization_result solve_root_newton_method(Func **funcs, std::size_t num_funcs, T *params,
                                                    std::size_t num_params,
                                                    double terminate_thresold,
                                                    std::size_t max_iteration) {
  optimization_result result;
...
static double merit(ResidualFunc **residuals, std::size_t num_residuals,
                    ...
                    EqConstraintFunc **eq_constraints, std::size_t num_eq_constraints, T *params,
                    std::size_t num_params, double *s, double rho, double eta) {
  T value = eval(residuals, num_residuals, params);
  value -= rho * std::accumulate(s, s + num_ineq_constraints, 0.0,
                                 [](double acc, double si) { return acc + std::log(si); });
...
static double backtracking_line_search(
    ResidualFunc **residuals, std::size_t num_residuals, InEqConstraintFunc **ineq_constraints,
    std::size_t num_ineq_constraints, EqConstraintFunc **eq_constraints,
    std::size_t num_eq_constraints, T *params, std::size_t num_params, double *s, double *u,
    T *dparams, double *ds, double *du, double rho, double eta, double beta) {
  double alpha1 = 1.0;
  double alpha2 = 1.0;
  ...
  while (alpha > initial_alpha * 0.1) {
    ...
    double m =
        merit(residuals, num_residuals, ineq_constraints, num_ineq_constraints, eq_constraints,
              num_eq_constraints, new_params_v.data(), num_params, new_s_v.data(), rho, eta);
...
template <typename ResidualFunc, typename InEqConstraintFunc, typename EqConstraintFunc, typename T>
static optimization_result solve_interior_point_method(
    ResidualFunc **residuals, std::size_t num_residuals, InEqConstraintFunc **ineq_constraints,
    std::size_t num_ineq_constraints, EqConstraintFunc **eq_constraints,
    std::size_t num_eq_constraints, T *params, std::size_t num_params, double terminate_thresold,
    std::size_t max_iteration, double eta = 0.1, double beta = 0.9, double t = 0.5) {
  optimization_result result;
  ...
  for (std::size_t iter = 0; iter < max_iteration; iter++) {
    ...
    Eigen::MatrixXd A_m = Eigen::MatrixXd::Zero(
    ...
    A_m.block(num_params + num_ineq_constraints, num_params, num_ineq_constraints,
    ...
    Eigen::VectorXd dv_v =
        delta_v.segment(num_params + 2 * num_ineq_constraints, num_eq_constraints);

    double alpha = backtracking_line_search(
        residuals, num_residuals, ineq_constraints, num_ineq_constraints, eq_constraints,
        num_eq_constraints, params_v.data(), num_params, s_v.data(), u_v.data(), dparams_v.data(),
        ds_v.data(), du_v.data(), rho, eta, beta);
