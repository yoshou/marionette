...
struct icp_3d_3d_minimizer {
  ...
  float update(std::size_t iter, const model_data &model, const frame_data_t &frame,
               const point_cloud &marker_cloud, const model_instance_data &registered_clusters,
               const std::unordered_map<std::size_t, glm::vec3> &estimated_pos,
               clusters_transform_params &params, bool with_twist, bool verbose = true);
};
