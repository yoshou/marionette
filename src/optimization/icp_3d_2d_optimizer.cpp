...
static void project_target(const model_instance_data &model_instance,
  ...
  for (std::size_t i = 0; i < registered_clusters.size(); i++) {
    ...
    for (std::size_t j = 0; j < registerd_points.size(); j++) {
      const auto marker =
          marionette::core::transform_coordinate(registerd_points[j].position, inverse_world);
      const auto pt = project_ortho(camera, marker);

      points.push_back(labeled_point{i, j, glm::vec2(pt / pt.z), pt.z});
    }
  }
}
...
static void project_target(const model_instance_data &model_instance,
  ...
  for (std::size_t i = 0; i < registered_clusters.size(); i++) {
    ...
    for (std::size_t j = 0; j < target_points.size(); j++) {
      const auto marker =
          marionette::core::transform_coordinate(target_points[j].position, inverse_world);
      ...
      const auto transformed_marker = transform_twist_axis_angle(
          marker, twist_angle, twist_axis, twist_weight, rotation, translation);
...
float icp_3d_2d_minimizer::update(std::size_t iter, const model_data &model,
                                  const std::vector<camera_t> &cameras, const frame_data_t &frame,
                                  const model_instance_data &model_instance, glm::mat4 world,
  ...
  for (std::size_t i = 0; i < registered_clusters.size(); i++) {
    ...
    const auto registerd_points =
        marionette::core::transform_coordinate(registered_cluster.target, inverse_world);
  ...
  for (std::size_t i = 0; i < observations.size(); ++i) {
    ...
    if (with_twist && std::abs(twist_weights[point_index[i]]) > 1e-5f) {
      ...
      const auto cluster_pose =
          transform_to_target_pose(registered_clusters[cluster_index[i]].cluster,
                                   registered_clusters[cluster_index[i]].fit_result);
      const auto twist_axis = to_line(inverse_world * cluster_pose);

      ceres::CostFunction *cost_function = twisted_rt_transform_projection_error::create(
          observation, point, view, proj, twist_weight, twist_axis);

      problem.AddResidualBlock(cost_function, loss, twist_angle, rotation, translation);
    } else {
      ceres::CostFunction *cost_function =
          rt_transform_projection_error::create(observation, point, view, proj);

      problem.AddResidualBlock(cost_function, loss, rotation, translation);
    }
  }
  ...
  const auto articulation_pairs = get_articulation_pairs();
  {
    ...
    for (const auto &camera : cameras) {
      ...
      for (std::size_t i = 0; i < articulation_pairs.size(); i++) {
        ...
        const auto [pos1, rot1, pos2, rot2] =
        ...
        const auto articulation_p =
            marionette::core::transform_coordinate((pos1 + pos2) * 0.5f, inverse_world);
        ...
        problem.AddResidualBlock(cost_function, loss, parent_rotation, parent_translation,
...

  {
    ...
    if (verbose) {
      ...
      for (std::size_t i = 0; i < num_cluster_cost_funcs.size(); i++) {
        std::cout << "Num cost funcs of cluster " << registered_clusters[i].cluster.name << " : "
                  << num_cluster_cost_funcs[i] << std::endl;
      }
    }
