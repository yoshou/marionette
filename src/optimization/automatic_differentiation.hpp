...
struct dual_t {
  ...
  dual_t() : a(0), b(0) {}
  inline explicit dual_t(const T &value) : a(value), b(0) {}
  inline dual_t(const T &a, const T &b) : a(a), b(b) {}
...
struct dual_vec_t {
  ...
  inline dual_vec_t() { v.setConstant(T()); }
  ...
  inline dual_vec_t(const T &a, const Eigen::DenseBase<Derived> &v) : a(a), v(v) {}
...
static inline dual_vec_t<T, N> sqrt(const dual_vec_t<T, N> &f) {
  ...
  const auto df_0 = sqrt(f.a);
  const auto df_1 = T{1} / (T{2} * df_0);
  return dual_vec_t<T, N>(df_0, f.v * df_1);
}
template <typename T, int N>
...
struct hyper_dual_t {
  ...
  hyper_dual_t() : a(0), b(0), c(0), d(0) {}
  hyper_dual_t(const hyper_dual_t<T> &other) : a(other.a), b(other.b), c(other.c), d(other.d) {}
  explicit hyper_dual_t(const T &value) : a(value), b(0), c(0), d(0) {}
  hyper_dual_t(const T &a, const T &b, const T &c, const T &d) : a(a), b(b), c(c), d(d) {}
...
template <typename T>
static inline hyper_dual_t<T> operator*(const hyper_dual_t<T> &f, const hyper_dual_t<T> &g) {
  return hyper_dual_t<T>(f.a * g.a, f.a * g.b + f.b * g.a, f.a * g.c + f.c * g.a,
...
template <typename T>
static inline hyper_dual_t<T> operator/(const T &s, const hyper_dual_t<T> &f) {
  const auto inv = hyper_dual_t<T>(T{1} / f.a, -f.b / (f.a * f.a), -f.c / (f.a * f.a),
...
static inline hyper_dual_t<T> sqrt(const hyper_dual_t<T> &f) {
  ...
  return hyper_dual_t<T>(df_0, f.b * df_1, f.c * df_1, f.d * df_1 + f.b * f.c * df_2);
}
template <typename T>
...
struct vec3_t<dual_t<float>> {
  ...
  vec3_t(const vec3_t<float> &a, const vec3_t<float> &b) : a(a), b(b) {}
};

static inline vec3_t<dual_t<float>> operator+(const vec3_t<dual_t<float>> &f) { return f; }
static inline vec3_t<dual_t<float>> operator-(const vec3_t<dual_t<float>> &f) {
  return vec3_t<dual_t<float>>(-f.a, -f.b);
}

static inline vec3_t<dual_t<float>> operator+(const vec3_t<dual_t<float>> &f,
                                              const vec3_t<float> &s) {
  return vec3_t<dual_t<float>>(f.a + s, f.b);
}
static inline vec3_t<dual_t<float>> operator+(const vec3_t<dual_t<float>> &f,
                                              const vec3_t<dual_t<float>> &g) {
...
static inline vec3_t<dual_t<float>> operator*(const vec3_t<float> &s,
                                              const vec3_t<dual_t<float>> &f) {
  return vec3_t<dual_t<float>>(f.a * s, f.b * s);
}
static inline vec3_t<dual_t<float>> operator*(const vec3_t<dual_t<float>> &f,
                                              const vec3_t<dual_t<float>> &g) {
...
static inline double l2norm(const vec3_t<double> &v) { return dot(v, v); }
...
struct vec3_t<dual_t<double>> {
  ...
  inline vec3_t(const dual_t<double> &x, const dual_t<double> &y, const dual_t<double> &z)
      : a(x.a, y.a, z.a), b(x.b, y.b, z.b) {}
  inline vec3_t(const vec3_t<double> &a, const vec3_t<double> &b) : a(a), b(b) {}
  inline explicit vec3_t(const vec3_t<double> &a) : a(a), b() {}

  inline dual_t<double> get_x() const { return dual_t<double>(a.x, b.x); }
  inline dual_t<double> get_y() const { return dual_t<double>(a.y, b.y); }
  inline dual_t<double> get_z() const { return dual_t<double>(a.z, b.z); }
};

static inline vec3_t<dual_t<double>> operator+(const vec3_t<dual_t<double>> &f) { return f; }
static inline vec3_t<dual_t<double>> operator-(const vec3_t<dual_t<double>> &f) {
  return vec3_t<dual_t<double>>(-f.a, -f.b);
}

static inline vec3_t<dual_t<double>> operator+(const vec3_t<dual_t<double>> &f,
                                               const vec3_t<double> &s) {
  return vec3_t<dual_t<double>>(f.a + s, f.b);
}
static inline vec3_t<dual_t<double>> operator+(const vec3_t<dual_t<double>> &f,
                                               const vec3_t<dual_t<double>> &g) {
...
static inline vec3_t<dual_t<double>> operator*(const vec3_t<double> &s,
                                               const vec3_t<dual_t<double>> &f) {
  return vec3_t<dual_t<double>>(f.a * s, f.b * s);
}
static inline vec3_t<dual_t<double>> operator*(const vec3_t<dual_t<double>> &f,
                                               const vec3_t<dual_t<double>> &g) {
...
static inline vec3_t<dual_t<double>> operator*(const vec3_t<dual_t<double>> &v0,
                                               const dual_t<double> &s) {
...
static inline vec3_t<dual_t<double>> cross(const vec3_t<dual_t<double>> &f,
                                           const vec3_t<dual_t<double>> &g) {
...
struct quat_t {
  ...
  quat_t() : x(), y(), z(), w() {}
  quat_t(T w, T x, T y, T z) : x(x), y(y), z(z), w(w) {}
};
...
static inline vec3_t<T> rotate(const quat_t<T> &q, const vec3_t<T> &v) {
  ...
  return vec3_t<T>(T(2) * ((t8 + t1) * v.x + (t6 - t4) * v.y + (t3 + t7) * v.z) + v.x,
...
#if 1
static inline vec3_t<dual_t<double>> rotate(const quat_t<dual_t<double>> &q,
                                            const vec3_t<dual_t<double>> &v) {
