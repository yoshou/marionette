...
void find_nearest_pair(const std::vector<weighted_point> &source_points,
                       const glm::mat4 &source_pose, const glm::mat4 &inv_pose,
                       const point_cloud &target_points, glm::vec3 translation, glm::mat3 rotation,
                       std::vector<std::pair<std::size_t, std::size_t>> &pairs, float twist_angle) {
  ...
  for (std::size_t i = 0; i < source_points.size(); i++) {
    ...
    const auto source_point =
        rotation * twist(source_points[i], source_pose, inv_pose, twist_angle) + translation;

    std::size_t num_points = target_points.knn_search(source_point, source_points.size(),
                                                      knn_index.data(), knn_distsq.data());
...
void find_nearest_pair(const std::vector<weighted_point> &source_points, const line3 &twist_axis,
                       const point_cloud &target_points, const glm::vec3 &translation,
  ...
  for (std::size_t i = 0; i < source_points.size(); i++) {
    const auto source_point =
        rotation * twist(source_points[i], twist_axis, twist_angle) + translation;
    std::size_t num_points = target_points.knn_search(source_point, source_points.size(),
                                                      knn_index.data(), knn_distsq.data());
...
static inline float mse(const std::vector<weighted_point> &source_points,
                        const glm::mat4 &source_pose, const glm::mat4 &inv_pose,
                        const point_cloud &target_points, const glm::vec3 &translation,
                        const glm::mat3 &rotation,
...
static inline float mse(const std::vector<weighted_point> &source_points, const line3 &twist_axis,
                        const point_cloud &target_points, const glm::vec3 &translation,
                        const glm::mat3 &rotation,
                        const std::vector<std::pair<std::size_t, std::size_t>> &pairs,
                        float twist_angle) {
...
static inline float mse(const std::vector<weighted_point> &source_points, const line3 &twist_axis,
                        ...
                        const glm::mat3 &rotation,
                        const std::vector<std::pair<std::size_t, std::size_t>> &pairs,
                        float twist_angle, const float *max_rotation_dists) {
  if (pairs.size() == 0) {
    return 0.f;
  }
...
static std::pair<glm::vec3, glm::vec3> centroid(
    const std::vector<weighted_point> &source_points, const line3 &twist_axis,
    const point_cloud &target_points, glm::vec3 translation, glm::mat3 rotation,
    const std::vector<std::pair<std::size_t, std::size_t>> &pairs, float twist_angle) {
...
static inline void solve_svd(const float *a, float *u, float *v, float *s, std::size_t m,
                             std::size_t n) {
...
#ifdef USE_CUDA
static void solve_svd_batched(const thrust::host_vector<float> &a, thrust::host_vector<float> &u,
                              thrust::host_vector<float> &v, thrust::host_vector<float> &s,
  ...
  for (std::size_t i = 0; i < batch_size; i++) {
    solve_svd(a.data() + i * m * n, u.data() + i * m * m, v.data() + i * n * n, s.data() + i * n, m,
              n);
  }
#endif
}
...
struct icp_minimizer {
  ...
  void update_rotation(const std::vector<weighted_point> &source_points, const line3 &twist_axis,
                       const point_cloud &target_points, const glm::vec3 &translation,
                       glm::mat3 &rotation, float twist_angle) {
  ...
  void update_translation(const std::vector<weighted_point> &source_points, const line3 &twist_axis,
                          const point_cloud &target_points, glm::vec3 &translation,
                          const glm::mat3 &rotation, float twist_angle) {
    const auto [centroid_a, centroid_b] = centroid(source_points, twist_axis, target_points,
                                                   translation, rotation, pairs, twist_angle);

    translation += (centroid_b - centroid_a);
  }

  void update_twist_angle(const std::vector<weighted_point> &source_points, const line3 &twist_axis,
                          const point_cloud &target_points, const glm::vec3 &translation,
                          const glm::mat3 &rotation, float &twist_angle, float min_twist_angle,
                          float max_twist_angle, float d_angle = 10.f) {
    const auto error1 =
        mse(source_points, twist_axis, target_points, translation, rotation, pairs, twist_angle);
    const auto error2 = mse(source_points, twist_axis, target_points, translation, rotation, pairs,
                            twist_angle + d_angle);

    twist_angle =
        std::max(min_twist_angle,
                 std::min(max_twist_angle, twist_angle - d_angle * (error2 - error1) / error1));
  }

  float compute(const std::vector<weighted_point> &source_points, const glm::mat4 &source_pose,
                const point_cloud &target_points, glm::vec3 &translation, glm::mat3 &rotation,
                float &twist_angle, float min_twist_angle, float max_twist_angle,
                std::uint32_t max_iter = 10, float threshold = 1e-4f) {
    float last_error = std::numeric_limits<float>::max();

    const auto twist_axis = to_line(source_pose);

    for (std::uint32_t iter = 0; iter < max_iter; iter++) {
      find_nearest_pair(source_points, twist_axis, target_points, translation, rotation, pairs,
                        twist_angle);

      update_rotation(source_points, twist_axis, target_points, translation, rotation, twist_angle);

      update_twist_angle(source_points, twist_axis, target_points, translation, rotation,
                         twist_angle, min_twist_angle, max_twist_angle);

      // update_translation(source_points, twist_axis, target_points, translation, rotation,
      // twist_angle);
...
static void compute_max_rotation_dists(const std::vector<weighted_point> &source_points,
  ...
  for (std::size_t l = 0; l < max_rotation_level; l++) {
    const auto sigma = 2.0 * glm::pi<double>() / std::pow(2.0, l) /
                       2.0;  // Half-side length of each level of rotation subcube
...
static std::tuple<float, glm::mat3, glm::vec3, float> estimate_rotation(
    const std::vector<weighted_point> &source_points, const glm::mat4 source_pose,
    const point_cloud &target_points, glm::vec3 initial_translation, float initial_twist_angle,
    float min_twist_angle, float max_twist_angle) {
...
static std::pair<triangle_feature, float> compute_feature(
    const std::vector<weighted_point> &source_points) {
  ...
  float feature_radius;
  {
    ...
    source_feature =
        extract_triangle_feature(anchor_pos, points, std::numeric_limits<float>::max());
  }
  return std::make_pair(source_feature, feature_radius);
}

static void find_fit(const std::vector<weighted_point> &source_points, const glm::mat4 &source_pose,
                     const point_cloud &target_points, float initial_twist_angle,
                     float min_twist_angle, float max_twist_angle,
  ...
  for (const auto &point : source_points) {
    rel_source_points.push_back(
        weighted_point{point.position - anchor_pos, point.weight, point.id});
  }
  ...
  for (std::size_t i = 0; i < target_points.size(); i++) {
    const auto feature =
        extract_triangle_feature(target_points[i], target_points.points, feature_radius * 1.2f);
    ...
    const auto [error, rotation, translation, twist_angle] =
        estimate_rotation(rel_source_points, rel_source_pose, target_points, initial_translation,
...
static void find_fit_batch(const std::vector<weighted_point> &source_points,
  ...
  for (std::uint32_t iter = 0; iter < max_iter; iter++) {
    for (auto &ctx : ctxs) {
      ...
      find_nearest_pair(rel_source_points, twist_axis, target_points, translation, rotation,
                        icp.pairs, twist_angle);

      // icp.update_translation(source_points, twist_axis, target_points, translation, rotation,
      // twist_angle);
    }
    ...
    for (auto &ctx : ctxs) {
      ...
      icp.update_rotation(rel_source_points, twist_axis, target_points, translation, rotation,
                          twist_angle);
    }
#else
    ...
    for (auto &ctx : ctxs) {
      ...
      icp.update_twist_angle(rel_source_points, twist_axis, target_points, translation, rotation,
                             twist_angle, min_twist_angle, max_twist_angle);
    }
    ...
    for (auto &ctx : ctxs) {
      ...
      ctx.error = mse(rel_source_points, twist_axis, target_points, translation, rotation,
                      icp.pairs, twist_angle);
...
void global_registration::find_fits(const rigid_cluster &cluster, const point_cloud &cloud,
                                    float initial_twist_angle, float min_twist_angle,
                                    float max_twist_angle, std::vector<find_fit_result> &results) {
  if (cluster.points.size() == 0) {
    return;
  }

  // find_fit_batch(cluster.points, cluster.pose, cloud, initial_twist_angle, min_twist_angle,
  // max_twist_angle, results);
  find_fit(cluster.points, cluster.pose, cloud, initial_twist_angle, min_twist_angle,
           max_twist_angle, results);

  std::sort(results.begin(), results.end(),
            [](const find_fit_result &a, const find_fit_result &b) { return a.error < b.error; });
}

void global_registration::find_fits(const rigid_cluster &cluster,
                                    const std::vector<glm::vec3> &points, float initial_twist_angle,
                                    float min_twist_angle, float max_twist_angle,
                                    std::vector<find_fit_result> &results) {
  if (cluster.points.size() == 0) {
    return;
