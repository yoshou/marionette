...
static auto solve_articuated_pair(const rigid_cluster &parent_cluster,
  ...
  for (std::size_t i = 0; i < parent_results.size(); i++) {
    ...
    const auto anchor_pos = twist(parent_cluster.points[0], parent_cluster.pose,
                                  glm::inverse(parent_cluster.pose), parent_result.twist_angle);
    const auto transform = glm::translate(glm::mat4(1.f), parent_result.translation) *
                           glm::mat4(parent_result.rotation) *
                           glm::translate(glm::mat4(1.f), -anchor_pos);
    ...
    for (std::size_t j = 0; j < child_results.size(); j++) {
      ...
      const auto anchor_pos = twist(child_cluster.points[0], child_cluster.pose,
                                    glm::inverse(child_cluster.pose), child_result.twist_angle);
      const auto transform = glm::translate(glm::mat4(1.f), child_result.translation) *
                             glm::mat4(child_result.rotation) *
                             glm::translate(glm::mat4(1.f), -anchor_pos);
...
typedef boost::adjacency_list<boost::listS, boost::vecS, boost::undirectedS, vertex_property,
                              edge_property>
    graph_t;
...
class my_visitor : boost::default_bfs_visitor {
 ...
 public:
  my_visitor(vertex_t dest) : dest(dest){};
...
std::tuple<std::map<std::string, std::size_t>, float, float> solve_articulation_chain_constraint(
    const model_data &model, const std::vector<articulation_results> &articulations,
    const std::string &from_cluster, const std::string &to_cluster) {
  graph_t g;
  ...
  for (std::size_t k = 0; k < articulations.size(); k++) {
    ...
    for (std::size_t i = 0; i < parent_results.size(); i++) {
      ...
      const auto anchor_pos = twist(parent_cluster.points[parent_result.anchor_index], angle_axis_a,
                                    parent_result.twist_angle);
      const auto transform_a = glm::translate(parent_result.translation) *
                               glm::mat4(parent_result.rotation) * glm::translate(-anchor_pos);
    ...
    for (std::size_t j = 0; j < child_results.size(); j++) {
      ...
      const auto anchor_pos = twist(child_cluster.points[child_result.anchor_index], angle_axis_b,
                                    child_result.twist_angle);
      const auto transform_b = glm::translate(child_result.translation) *
                               glm::mat4(child_result.rotation) * glm::translate(-anchor_pos);
  ...
  if (parents[to] == to) {
    return std::make_tuple(std::map<std::string, std::size_t>{}, std::numeric_limits<float>::max(),
...
std::tuple<glm::vec3, glm::mat4, glm::vec3, glm::mat4> compute_articuated_pair_points(
  ...
  const auto articulation_rot = child_cluster.pose;
  {
    const auto anchor_pos = twist(parent_cluster.points[0], parent_cluster.pose,
                                  glm::inverse(parent_cluster.pose), parent_result.twist_angle);
    const auto transform = glm::translate(parent_result.translation) *
                           glm::mat4(parent_result.rotation) *
                           glm::translate(glm::mat4(1.f), -anchor_pos);
...

  {
    const auto anchor_pos = twist(child_cluster.points[0], child_cluster.pose,
                                  glm::inverse(child_cluster.pose), child_result.twist_angle);
    const auto transform = glm::translate(child_result.translation) *
                           glm::mat4(child_result.rotation) *
                           glm::translate(glm::mat4(1.f), -anchor_pos);
  ...
  return std::make_tuple(articulation_pos_a, articulation_rot_a, articulation_pos_b,
                         articulation_rot_b);
}
