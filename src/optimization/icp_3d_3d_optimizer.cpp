...
class closest_point_correnspondance_matcher final : public correnspondance_matcher {
 public:
  virtual std::vector<std::vector<std::pair<std::pair<std::size_t, std::size_t>, float>>>
  find_correnspondance(const std::vector<std::vector<weighted_point>>& sources,
                       const std::vector<std::vector<weighted_point>>& search_points,
                       const point_cloud& target) override {
    std::vector<std::vector<std::pair<std::pair<std::size_t, std::size_t>, float>>> results(
        sources.size());
 ...
 public:
  ...
  find_correnspondance(const std::vector<std::vector<weighted_point>>& sources,
    ...
    for (std::size_t cluster = 0; cluster < sources.size(); cluster++) {
      ...
      for (std::size_t i = 0; i < source.size(); i++) {
        target.radius_search(search_point[i].position, search_radius * search_radius,
                             candidates[i]);
      }
        ...
        for (std::size_t i = 0; i < candidates[mi].size(); i++) {
          ...
          for (std::size_t mj = 0; mj < source.size(); mj++) {
            ...
            for (std::size_t j = 0; j < candidates[mj].size(); j++) {
              ...
              const auto len_diff = glm::distance(source[mi].position, source[mj].position) -
                                    glm::distance(target[ui], target[uj]);
              const auto angle_diff =
                  1.0f -
                  glm::dot(source[mi].position - source[mj].position, target[ui] - target[uj]) /
                      (glm::length(source[mi].position - source[mj].position) *
                       glm::length(target[ui] - target[uj]));
...
icp_3d_3d_minimizer::icp_3d_3d_minimizer()
    : matcher(new closest_point_correnspondance_matcher()), elapsed(0.0) {}

float icp_3d_3d_minimizer::update(std::size_t iter, const model_data& model,
                                  const frame_data_t& frame, const point_cloud& marker_cloud,
                                  const model_instance_data& model_instance,
                                  const std::unordered_map<std::size_t, glm::vec3>& estimated_pos,
                                  clusters_transform_params& params, bool with_twist,
                                  bool verbose) {
...

  {
    ...
    for (std::size_t i = 0; i < registered_clusters.size(); i++) {
      ...
      for (std::size_t j = 0; j < registered_cluster.target.size(); j++) {
        ...
        source_point.position = transform_twist_quat(point.position, twist_angle, twist_axis,
                                                     point.weight, rotation, translation);
    ...
    for (std::size_t i = 0; i < registered_clusters.size(); i++) {
      for (const auto& p : pairs[i]) {
        ...
        residual_datas[i].push_back(residual_data{point, closest_point, source_index, target_index,
                                                  source_points[i][pair.first].weight, dist, 1.0f});
...

  {
    ...
    for (std::size_t i = 0; i < num_clusters; i++) {
      ...
      const auto cluster_pose = transform_to_target_pose(registered_clusters[i].cluster,
                                                         registered_clusters[i].fit_result);
      ...
      for (std::size_t j = 0; j < residual_datas[i].size(); j++) {
        ...
        if (!use_quaternion) {
          if (use_twist) {
            ceres::CostFunction* cost_function = twisted_rt_transform_error::create(
                source_point, target_point, twist_weight, twist_axis);

            problem.AddResidualBlock(cost_function, loss, twist_angle_param, rotation_param,
                                     translation_param);
          } else {
            ceres::CostFunction* cost_function =
                rt_transform_error::create(source_point, target_point);

            problem.AddResidualBlock(cost_function, loss, rotation_param, translation_param);
          }
        } else {
          if (use_twist) {
            ceres::CostFunction* cost_function = quat_twisted_rt_transform_error::create(
                source_point, target_point, twist_weight, twist_axis);
      ...
      std::transform(residual_datas[i].begin(), residual_datas[i].end(),
                     std::back_inserter(source_points),
                     [](const auto& res) { return res.source_point; });

      std::vector<glm::vec3> target_points;
      std::transform(residual_datas[i].begin(), residual_datas[i].end(),
                     std::back_inserter(target_points),
                     [](const auto& res) { return res.target_point; });
      std::vector<double> twist_weights;
      std::transform(residual_datas[i].begin(), residual_datas[i].end(),
                     std::back_inserter(twist_weights),
                     [](const auto& res) { return res.twist_weight; });
      std::vector<double> weights;
      std::transform(residual_datas[i].begin(), residual_datas[i].end(),
                     std::back_inserter(weights),
                     [](const auto& res) { return res.weight_in_cluster; });
      ...
      if (!use_quaternion) {
        if (use_twist) {
          const auto functor = twisted_rt_least_square_functor(source_points, target_points,
                                                               twist_weights, twist_axis, weights);
          const auto residual = optimization::make_auto_diff_function_term(
              functor, rotate_param, translate_param, twist_angle_param);
          residuals.push_back(residual);
        } else {
          const auto functor = rt_least_square_functor(source_points, target_points, weights);
          const auto residual =
              optimization::make_auto_diff_function_term(functor, rotate_param, translate_param);
          residuals.push_back(residual);
        }
      } else {
        if (use_twist) {
          const auto functor = quat_twisted_rt_least_square_functor(
              source_points, target_points, twist_weights, twist_axis, weights);
          const auto residual = optimization::make_auto_diff_function_term(
              functor, rotate_param, translate_param, twist_angle_param);
          residuals.push_back(residual);
        } else {
...
#if 1
    {
      ...
      for (std::size_t i = 0; i < articulation_pairs.size(); i++) {
        ...
        auto [pos1, rot1, pos2, rot2] =
            compute_articuated_pair_points(parent_cluster.cluster, child_cluster.cluster,
                                           parent_cluster.fit_result, child_cluster.fit_result);
        ...
        const auto parent_rotation_param =
            &rotation_params[parent_cluster_idx * rotation_param_size];
        ...
        {
          const auto idx = prob["residual_blocks"].size();
          prob["residual_blocks"][idx]["params"]["parent_rotation"] =
              parent_cluster_idx * rotation_param_size;
          prob["residual_blocks"][idx]["params"]["parent_translation"] = parent_cluster_idx * 3;
          prob["residual_blocks"][idx]["params"]["child_rotation"] =
              child_cluster_idx * rotation_param_size;
        ...
        if (use_quaternion) {
          ceres::CostFunction* cost_function = quat_articulation_point_error::create(pos1, pos2);

          problem.AddResidualBlock(cost_function, loss, parent_rotation_param,
                                   parent_translation_param, child_rotation_param,
                                   child_translation_param);
        } else {
          ceres::CostFunction* cost_function = articulation_point_error::create(pos1, pos2);
        ...
        if (use_quaternion) {
          ...
          const auto constraint = optimization::make_auto_diff_function_term(
              functor, parent_rotate_param, parent_translate_param, child_rotate_param,
              child_translate_param);
...

    {
      for (std::size_t i = 0; i < model.constraints.size(); i++) {
        ...
        const auto parent_cluster_pose =
            transform_to_target_pose(parent_cluster.cluster, parent_cluster.fit_result);
        ...
        pos1 = transform_twist_quat(pos1, parent_twist_angle, parent_twist_axis,
                                    parent_twist_weight, parent_rotation, parent_translation);
        ...
        const auto child_cluster_pose =
            transform_to_target_pose(child_cluster.cluster, child_cluster.fit_result);
        auto child_twist_axis = to_line(child_cluster_pose);

        const auto child_twist_weight = child_cluster.target[point2.index].weight;
        pos2 = transform_twist_quat(pos2, child_twist_angle, child_twist_axis, child_twist_weight,
        ...
        if (!use_quaternion) {
          if (use_twist) {
            ceres::CostFunction* cost_function = twist_articulation_point_error::create(
                pos1, pos2, parent_twist_weight, parent_twist_axis, child_twist_weight,
                child_twist_axis);

            problem.AddResidualBlock(cost_function, loss, parent_twist_angle_param,
                                     parent_rotation_param, parent_translation_param,
                                     child_twist_angle_param, child_rotation_param,
                                     child_translation_param);
          } else {
            ceres::CostFunction* cost_function = articulation_point_error::create(pos1, pos2);
          ...
          if (use_twist) {
            ceres::CostFunction* cost_function = quat_twist_articulation_point_error::create(
                pos1, pos2, parent_twist_weight, parent_twist_axis, child_twist_weight,
                child_twist_axis);
        ...
        if (!use_quaternion) {
          if (use_twist) {
            const auto functor = twisted_rt_articulation_least_square_constraint(
                pos1, pos2, parent_twist_weight, child_twist_weight, parent_twist_axis,
                child_twist_axis);
            const auto constraint = optimization::make_auto_diff_function_term(
                functor, parent_rotate_param, parent_translate_param, parent_twist_angle_param,
                child_rotate_param, child_translate_param, child_twist_angle_param);
            residuals.push_back(constraint);
          } else {
            const auto functor = rt_articulation_least_square_constraint(pos1, pos2);
            const auto constraint = optimization::make_auto_diff_function_term(
          ...
          if (use_twist) {
            const auto functor = quat_twisted_rt_articulation_least_square_constraint(
                pos1, pos2, parent_twist_weight, child_twist_weight, parent_twist_axis,
                child_twist_axis);
            const auto constraint = optimization::make_auto_diff_function_term(
                functor, parent_rotate_param, parent_translate_param, parent_twist_angle_param,
...

  {
...

    {
      ...
      for (std::size_t i = 0; i < num_clusters; i++) {
        ...
        const auto to_vec3 = [](const double* values) {
          return glm::dvec3(static_cast<double>(values[0]), static_cast<double>(values[1]),
                            static_cast<double>(values[2]));
        };

        const auto to_quat = [](const double* values) {
          return glm::normalize(
              glm::dquat(static_cast<double>(values[0]), static_cast<double>(values[1]),
        ...
        if (!use_quaternion) {
          ...
          delta_rotation = (std::abs(delta_len) <= std::numeric_limits<float>::epsilon())
                               ? glm::dquat(1.0, 0.0, 0.0, 0.0)
                               : glm::angleAxis(delta_len, glm::normalize(delta_angle_axis));
        } else {
        ...
        from_vec3(glm::angle(current_transform.rotation) *
                      glm::normalize(glm::axis(current_transform.rotation)),
                  mutable_rotation);
...

    {
      ...
      for (std::size_t i = 0; i < num_clusters; i++) {
        ...
        const auto rotation = (std::abs(len) <= std::numeric_limits<float>::epsilon())
                                  ? glm::dquat(1.0, 0.0, 0.0, 0.0)
                                  : glm::angleAxis(len, glm::normalize(angle_axis));
        const auto translation = to_vec3(mutable_translation);
