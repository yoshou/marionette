...
static void transform_coordinate(const T *p, const glm::mat4 &m, T *result) {
  ...
  result[2] = ((double)m[0][2] * p[0]) + ((double)m[1][2] * p[1]) + ((double)m[2][2] * p[2]) +
              (double)m[3][2];
  result[3] = ((double)m[0][3] * p[0]) + ((double)m[1][3] * p[1]) + ((double)m[2][3] * p[2]) +
              (double)m[3][3];
}
...
inline void rotate(const T axis[3], const T theta, const T pt[3], T result[3]) {
  if (theta > T(std::numeric_limits<double>::epsilon())) {
    ...
    const T w_cross_pt[3] = {axis[1] * pt[2] - axis[2] * pt[1], axis[2] * pt[0] - axis[0] * pt[2],
                             axis[0] * pt[1] - axis[1] * pt[0]};
    const T tmp = (axis[0] * pt[0] + axis[1] * pt[1] + axis[2] * pt[2]) * (T(1.0) - costheta);
...
struct rt_transform_projection_error {
  ...
  rt_transform_projection_error(const glm::vec2 &observed, const glm::vec3 &point,
                                const glm::mat4 &view, const glm::mat3 &proj)
  ...
  bool operator()(const T *const rotation, const T *const translation, T *residuals) const {
    ...
    T point[3] = {T(static_cast<double>(this->point.x)), T(static_cast<double>(this->point.y)),
  ...
  static ceres::CostFunction *create(const glm::vec2 &observed, const glm::vec3 &point,
                                     const glm::mat4 &view, const glm::mat3 &proj) {
...
struct twisted_rt_transform_projection_error {
  ...
  twisted_rt_transform_projection_error(const glm::vec2 &observed, const glm::vec3 &point,
                                        const glm::mat4 &view, const glm::mat3 &proj,
                                        float twist_weight, const line3 &twist_axis)
      : observed(observed),
        point(point),
        view_proj(glm::mat4(proj) * view),
        twist_weight(twist_weight),
        twist_axis(twist_axis) {}

  template <typename T>
  bool operator()(const T *const twist_angle, const T *const rotation, const T *const translation,
                  T *residuals) const {
    if (observed.x < 0) {
      return false;
    }
    ...
    twist_angle_axis[0] =
        static_cast<double>(twist_axis.direction.x * twist_weight) * twist_angle[0];
    twist_angle_axis[1] =
        static_cast<double>(twist_axis.direction.y * twist_weight) * twist_angle[0];
    twist_angle_axis[2] =
        static_cast<double>(twist_axis.direction.z * twist_weight) * twist_angle[0];

    T point[3] = {T(static_cast<double>(this->point.x - this->twist_axis.origin.x)),
  ...
  static ceres::CostFunction *create(const glm::vec2 &observed, const glm::vec3 &point,
    ...
    return (new ceres::AutoDiffCostFunction<twisted_rt_transform_projection_error, 2, 1, 3, 3>(
        new twisted_rt_transform_projection_error(observed, point, view, proj, twist_weight,
                                                  twist_axis)));
  }
};
...
struct articulation_projection_error {
  ...
  articulation_projection_error(const glm::vec3 &point, const glm::mat4 &view,
                                const glm::mat3 &proj)
  ...
  static ceres::CostFunction *create(const glm::vec3 &point, const glm::mat4 &view,
                                     const glm::mat3 &proj) {
...
struct twisted_rt_transform_error {
  ...
  twisted_rt_transform_error(const glm::vec3 &point, const glm::vec3 &target, float twist_weight,
                             const line3 &twist_axis)
...
struct articulation_point_error {
  ...
  bool operator()(const T *const rotation1, const T *const translation1, const T *const rotation2,
                  const T *const translation2, T *residuals) const {
    T point1[3] = {T(static_cast<double>(this->point1.x)), T(static_cast<double>(this->point1.y)),
                   T(static_cast<double>(this->point1.z))};
    T point2[3] = {T(static_cast<double>(this->point2.x)), T(static_cast<double>(this->point2.y)),
...
struct twist_articulation_point_error {
  ...
  twist_articulation_point_error(glm::vec3 point1, glm::vec3 point2, float twist_weight1,
                                 const line3 &twist_axis1, float twist_weight2,
                                 const line3 &twist_axis2)
      : point1(point1),
        point2(point2),
        twist_weight1(twist_weight1),
        twist_axis1(twist_axis1),
        twist_weight2(twist_weight2),
        twist_axis2(twist_axis2) {}

  template <typename T>
  bool operator()(const T *const twist_angle1, const T *const rotation1,
                  const T *const translation1, const T *const twist_angle2,
                  const T *const rotation2, const T *const translation2, T *residuals) const {
    T twist_angle_axis1[3];
    twist_angle_axis1[0] =
        static_cast<double>(twist_axis1.direction.x * twist_weight1) * twist_angle1[0];
    twist_angle_axis1[1] =
        static_cast<double>(twist_axis1.direction.y * twist_weight1) * twist_angle1[0];
    twist_angle_axis1[2] =
        static_cast<double>(twist_axis1.direction.z * twist_weight1) * twist_angle1[0];

    T twist_angle_axis2[3];
    twist_angle_axis2[0] =
        static_cast<double>(twist_axis2.direction.x * twist_weight2) * twist_angle2[0];
    twist_angle_axis2[1] =
        static_cast<double>(twist_axis2.direction.y * twist_weight2) * twist_angle2[0];
    twist_angle_axis2[2] =
        static_cast<double>(twist_axis2.direction.z * twist_weight2) * twist_angle2[0];

    T point1[3] = {T(static_cast<double>(this->point1.x - this->twist_axis1.origin.x)),
                   T(static_cast<double>(this->point1.y - this->twist_axis1.origin.y)),
                   T(static_cast<double>(this->point1.z - this->twist_axis1.origin.z))};

    T point2[3] = {T(static_cast<double>(this->point2.x - this->twist_axis2.origin.x)),
  ...
  static ceres::CostFunction *create(glm::vec3 point1, glm::vec3 point2, float twist_weight1,
                                     const line3 &twist_axis1, float twist_weight2,
                                     const line3 &twist_axis2) {
    return (new ceres::AutoDiffCostFunction<twist_articulation_point_error, 3, 1, 3, 3, 1, 3, 3>(
        new twist_articulation_point_error(point1, point2, twist_weight1, twist_axis1,
                                           twist_weight2, twist_axis2)));
  }
};
...
struct quat_twist_articulation_point_error {
  ...
  quat_twist_articulation_point_error(glm::vec3 point1, glm::vec3 point2, float twist_weight1,
                                      const line3 &twist_axis1, float twist_weight2,
                                      const line3 &twist_axis2)
      : point1(point1),
        point2(point2),
  ...
  static ceres::CostFunction *create(glm::vec3 point1, glm::vec3 point2, float twist_weight1,
                                     ...
                                     const line3 &twist_axis2) {
    return (
        new ceres::AutoDiffCostFunction<quat_twist_articulation_point_error, 3, 1, 4, 3, 1, 4, 3>(
            new quat_twist_articulation_point_error(point1, point2, twist_weight1, twist_axis1,
                                                    twist_weight2, twist_axis2)));
  }
};
...
struct srt_least_square_functor {
  ...
  srt_least_square_functor(const std::vector<glm::vec3> &points,
                           const std::vector<glm::vec3> &target_points, const glm::vec3 &origin)
      : points(points), target_points(target_points), origin(origin) {}
  template <typename T>
  bool operator()(const T *const scale, const T *const rotation, const T *const translation,
                  T *residuals) const {
    T residual(0);
    ...
    for (std::size_t i = 0; i < points.size(); i++) {
      ...
      T point[3] = {T(static_cast<double>(pt.x - origin.x)),
      ...
      residual += (residuals[0] * residuals[0] + residuals[1] * residuals[1] +
...
struct srt_articulation_least_square_constraint {
  ...
  srt_articulation_least_square_constraint(glm::vec3 point1, glm::vec3 point2,
                                           glm::vec3 parent_origin, glm::vec3 child_origin)
      : point1(point1), point2(point2), parent_origin(parent_origin), child_origin(child_origin) {}

  template <typename T>
  bool operator()(const T *const scale1, const T *const rotation1, const T *const translation1,
                  const T *const scale2, const T *const rotation2, const T *const translation2,
                  T *residuals) const {
    T point1[3] = {T(static_cast<double>(this->point1.x - this->parent_origin.x)),
                   T(static_cast<double>(this->point1.y - this->parent_origin.y)),
                   T(static_cast<double>(this->point1.z - this->parent_origin.z))};
    T point2[3] = {T(static_cast<double>(this->point2.x - this->child_origin.x)),
...
struct sr_least_square_functor {
  ...
  sr_least_square_functor(const std::vector<glm::vec3> &points,
                          const std::vector<glm::vec3> &target_points, const glm::vec3 &origin)
      : points(points), target_points(target_points), origin(origin) {}
  template <typename T>
  bool operator()(const T *const scale, const T *const rotation, T *residuals) const {
    T residual(0);
...
struct sr_articulation_least_square_constraint {
  ...
  sr_articulation_least_square_constraint(glm::vec3 point1, glm::vec3 point2,
                                          glm::vec3 parent_origin, glm::vec3 child_origin)
      : point1(point1), point2(point2), parent_origin(parent_origin), child_origin(child_origin) {}
  template <typename T>
  bool operator()(const T *const scale1, const T *const rotation1, const T *const scale2,
                  const T *const rotation2, T *residuals) const {
    T point1[3] = {T(static_cast<double>(this->point1.x - this->parent_origin.x)),
                   T(static_cast<double>(this->point1.y - this->parent_origin.y)),
                   T(static_cast<double>(this->point1.z - this->parent_origin.z))};
    T point2[3] = {T(static_cast<double>(this->point2.x - this->child_origin.x)),
...
struct rt_least_square_functor {
  ...
  rt_least_square_functor(const std::vector<glm::vec3> &points,
  ...
  bool operator()(const T *const rotation, const T *const translation, T *residuals) const {
    ...
    for (std::size_t i = 0; i < points.size(); i++) {
      ...
      T point[3] = {T(static_cast<double>(pt.x)), T(static_cast<double>(pt.y)),
      ...
      residual += weights[i] * (residuals[0] * residuals[0] + residuals[1] * residuals[1] +
                                residuals[2] * residuals[2]);
    }
  ...
  template <typename T>
  inline T operator()(const T *const rotation, const T *const translation) const {
    T residual(0);
...
struct twisted_rt_least_square_functor {
  ...
  twisted_rt_least_square_functor(const std::vector<glm::vec3> &points,
                                  const std::vector<glm::vec3> &target_points,
                                  const std::vector<double> &twist_weights, const line3 &twist_axis,
                                  const std::vector<double> &weights)
      : points(points),
  ...
  inline T operator()(const T *const rotation, const T *const translation,
    ...
    for (std::size_t i = 0; i < points.size(); i++) {
      ...
      T point[3] = {T(static_cast<double>(source.x - twist_axis.origin.x)),
      ...
      residual += 1.0 * (residuals[0] * residuals[0] + residuals[1] * residuals[1] +
                         residuals[2] * residuals[2]);
    }
...
struct quat_rt_least_square_functor {
  ...
  quat_rt_least_square_functor(const std::vector<glm::vec3> &points,
                               const std::vector<glm::vec3> &target_points,
                               const std::vector<double> &weights)
      : points(points), target_points(target_points), weights(weights) {}
...
struct quat_twisted_rt_least_square_functor {
  ...
  quat_twisted_rt_least_square_functor(const std::vector<glm::vec3> &points,
                                       const std::vector<glm::vec3> &target_points,
                                       const std::vector<double> &twist_weights,
  ...
  inline T operator()(const T *const rotation, const T *const translation,
    ...
    const vec3_t<T> origin_vec(T(twist_axis.origin.x), T(twist_axis.origin.y),
                               T(twist_axis.origin.z));
    const vec3_t<T> axis_vec(T(twist_axis.direction.x), T(twist_axis.direction.y),
                             T(twist_axis.direction.z));
    ...
    for (std::size_t i = 0; i < points.size(); i++) {
      ...
      const auto point_vec =
          rotate(axis_vec, T(twist_weight) * twist_angle[0], pt_vec - origin_vec) + origin_vec;
#endif
...
struct rt_articulation_least_square_constraint {
  ...
  template <typename T>
  inline T operator()(const T *const rotation1, const T *const translation1,
                      const T *const rotation2, const T *const translation2) const {
    T point1[3] = {T(static_cast<double>(this->point1.x)), T(static_cast<double>(this->point1.y)),
                   T(static_cast<double>(this->point1.z))};
    T point2[3] = {T(static_cast<double>(this->point2.x)), T(static_cast<double>(this->point2.y)),
                   T(static_cast<double>(this->point2.z))};
    ...
    return 5.0 * (constraints[0] * constraints[0] + constraints[1] * constraints[1] +
                  constraints[2] * constraints[2]);
  }
  ...
  bool operator()(const T *const rotation1, const T *const translation1, const T *const rotation2,
    ...
    residuals[0] = 5.0 * (constraints[0] * constraints[0] + constraints[1] * constraints[1] +
                          constraints[2] * constraints[2]);
...
struct quat_rt_articulation_least_square_constraint {
  ...
  quat_rt_articulation_least_square_constraint(glm::vec3 point1, glm::vec3 point2,
                                               double weight = 5.0)
...
struct twisted_rt_articulation_least_square_constraint {
  ...
  twisted_rt_articulation_least_square_constraint(glm::vec3 point1, glm::vec3 point2,
                                                  double twist_weight1, double twist_weight2,
                                                  const line3 &twist_axis1,
                                                  const line3 &twist_axis2)
      : point1(point1),
        point2(point2),
  ...
  template <typename T>
  inline T operator()(const T *const rotation1, const T *const translation1,
                      const T *const twist_angle1, const T *const rotation2,
                      const T *const translation2, const T *const twist_angle2) const {
    T point1[3] = {T(static_cast<double>(this->point1.x - twist_axis1.origin.x)),
                   T(static_cast<double>(this->point1.y - twist_axis1.origin.y)),
                   T(static_cast<double>(this->point1.z - twist_axis1.origin.z))};
    T point2[3] = {T(static_cast<double>(this->point2.x - twist_axis2.origin.x)),
...
struct quat_twisted_rt_articulation_least_square_constraint {
  ...
  quat_twisted_rt_articulation_least_square_constraint(glm::vec3 point1, glm::vec3 point2,
                                                       double twist_weight1, double twist_weight2,
                                                       const line3 &twist_axis1,
                                                       const line3 &twist_axis2)
      : point1(point1),
