...
std::vector<std::string> serial_port::get_serial_port_names()
{
    ...
    while (SetupDiEnumDeviceInfo(hinfo, index, &info_data))
    {
        ...
        if (SetupDiGetDeviceRegistryProperty(
                hinfo, &info_data, SPDRP_FRIENDLYNAME, &type, (PBYTE)buff, MAX_PATH, &size) == TRUE)
        {
            fullname = buff;
        }
        else if (SetupDiGetDeviceRegistryProperty(
                     hinfo, &info_data, SPDRP_DEVICEDESC, &type, (PBYTE)buff, MAX_PATH, &size) == TRUE)
        {
            fullname = buff;
        }
...
serial_port::serial_port()
    : handle(std::make_unique<handle_t>())
{
}
...
{
    handle->value = CreateFileA((std::string(PATH) + name).c_str(), GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
static bool is_console(const std::string &driver)
{
    return driver == "serial8250";
}
