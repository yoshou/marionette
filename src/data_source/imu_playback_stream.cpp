...
imu_playback_stream::imu_playback_stream(std::string directory, uint32_t fps,
...
void imu_playback_stream::subscribe_quat(
  ...
  while (frame_no < frame_numbers.size()) {
    std::string path =
        (fs::path(directory) / (std::to_string(frame_numbers[frame_no]) + ".json")).string();
