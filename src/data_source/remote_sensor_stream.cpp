...
remote_sensor_stream::remote_sensor_stream(std::string endpoint)
    : stub_(
          stargazer::Sensor::NewStub(grpc::CreateChannel(endpoint, create_channel_credentials()))) {
}

void remote_sensor_stream::subscribe_sphere(
    const std::string& name, std::function<void(const std::vector<glm::vec3>&)> callback) {
...
void remote_sensor_stream::subscribe_quat(
    const std::string& name, std::function<void(const std::vector<glm::quat>&)> callback) {
  grpc::ClientContext context;
  stargazer::SubscribeRequest request;
  request.set_name(name);
