...
template <typename T>
static void write_point_image(std::string path, const std::vector<T> &points, std::size_t width,
                              std::size_t height) {
...
template <typename T>
static void write_point_image(std::string path, const std::vector<T> &points1,
                              const std::vector<T> &points2, std::size_t width,
                              std::size_t height) {
  cv::Mat image = cv::Mat::zeros(height, width, CV_8UC3);
...
template <typename T>
static void write_paired_point_image(std::string path, const std::vector<T> &points1,
                                     const std::vector<T> &points2, std::size_t width,
                                     std::size_t height) {
  cv::Mat image = cv::Mat::zeros(height, width, CV_8UC3);
  for (std::size_t i = 0; i < points1.size(); i++) {
    ...
    cv::line(image, cv::Point(point1.p.x, point1.p.y), cv::Point(point2.p.x, point2.p.y),
